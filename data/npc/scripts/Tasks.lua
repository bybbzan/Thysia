...
local function greetCallback(cid)
        ...
        if player:getStorageValue(JOIN_STOR) == -1 then
                npcHandler:setMessage(MESSAGE_GREET, 'Welcome ' .. player:getName() .. '. Say {join} if you want to join our task society.')
...
local function creatureSayCallback(cid, type, msg)
        ...
        elseif msgcontains('join', msg) or msgcontains('yes', msg) and npcHandler.topic[cid] == 5 then
                ...
                npcHandler:say('Great!, now you can start doing tasks.', cid) --I'm not sure if this is as real tibia. I let this piece of code because it was in the original file.
                npcHandler.topic[cid] = 0
        elseif isInArray({'tasks', 'task', 'mission'}, msg:lower()) then
                local can = player:getTasks()
                if player:getStorageValue(JOIN_STOR) == -1 then
                        return npcHandler:say('You\'ll have to {join}, to start any {tasks}.',cid)
                end
                if #can > 0 then
                        local text = ''
                        local sep = ', '
                        ...
                        local t = 0
                        local modalWindow = ModalWindow(1, 'Task System', 'Select a task:')
                        for _, id in ipairs(can) do
                                ...
                                modalWindow:addChoice(id, tasks[id].name or tasks[id].raceName)
                        end
                        ...
                        npcHandler:say('I don\'t have any tasks for you right now.', cid)
                end
        elseif msgcontains('status', msg) then
                ...
                if started and #started > 0 then
                        ...
                        for _, id in ipairs(started) do
                                ...
                                text = text .. ' ' .. tasks[id].raceName .. ', Current kills: ' .. player:getStorageValue(KILLSSTORAGE_BASE + id) .. '/' .. tasks[id].killsRequired .. '.\n'
                        end
        ...
        elseif msgcontains('report', msg) then
                ...
                if started and #started > 0 then
                        ...
                        if not finishedAtLeastOne then
                                ...
                                npcHandler:say('Awesome! you finished ' .. (finished > 1 and 'various' or 'a') .. ' task' .. (finished > 1 and 's' or '') .. '. Talk to me again if you want to start a {task}. If you want to face the boss, just say {boss}.', cid)
                        end
                else
                        npcHandler:say('You haven\'t started any task yet.', cid)
                end
        ...
        elseif isInArray({'points', 'rank'}, msg:lower()) then
                ...
                npcHandler.topic[cid] = 0
        elseif isInArray({'1234sfaq2341'}, msg:lower()) then
                ...
                elseif msg:lower() == 'boss' then
                        npcHandler:say('Are you ready to face the boss now?', cid)
                        npcHandler.topic[cid] = 6
                elseif msg:lower() == 'yes' and npcHandler.topic[cid] == 6 then
                        local destination = Position(898, 867, 8)
